...
public class KhasengerPanel extends JPanel 
{
	...
	public KhasengerPanel(ProgUI pui)
	{
		...
		this.convoScrollerRefresher = new Runnable() { public void run() { convoScroller.revalidate(); convoScroller.repaint(); } };
		this.futureCancel = timers.scheduleWithFixedDelay(convoScrollerRefresher, 0, 5, TimeUnit.MILLISECONDS);
	...
	{
		this.ncl = null;
		this.futureCancel.cancel(true);
		this.timers.shutdownNow();
		this.convoPane.setText("[Khasenger v1.0 by Zaineyy]\n\n");
		System.gc();
	}
	...
	public void restart()
	{
		this.ncl = this.parent.getNetClient();
		this.timers = Executors.newSingleThreadScheduledExecutor(new TimerNamer());
		this.futureCancel = timers.scheduleWithFixedDelay(this.convoScrollerRefresher, 0, 5, TimeUnit.MILLISECONDS);
	}
	
	public NetworkClient getNetClient() { return this.ncl; }
